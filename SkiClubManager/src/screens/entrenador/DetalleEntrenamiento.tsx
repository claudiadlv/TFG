...
import { View, Text, Alert, TouchableOpacity, ImageBackground, StyleSheet, ScrollView, ActivityIndicator } from 'react-native';
...
type Props = {
  route: RouteProp<EntrenadorStackParamList, 'DetalleEntrenamiento'>;
};
...
export default function DetalleEntrenamiento({ route }: Props) {
  ...
  const handleEliminar = async () => {
    Alert.alert('Eliminar', '¿Seguro?', [
      ...
      { text: 'Sí', onPress: async () => {
          const res = await fetch(`${API_URL}/eventos/${entrenamiento.id}`, { method: 'DELETE' });
  ...
  return (
    <ScrollView style={styles.container}>
      ...
      <View style={[styles.info, { paddingBottom: 10 }]}>
        ...
        {tieneTransporte && (
          ...
          >
            ...
            <View style={{ flex: 1 }}>
              ...
              {/* 🚀 Modificado el subtítulo con un \n para saltar la línea elegantemente */}
              <Text style={[cardStyles.subtitulo, { color: estaLlenoReal ? '#B91C1C' : '#0C4A6E', marginTop: 2 }]}>
                {`${ocupadas} de ${plazasTotales} plazas ocupadas`}{entrenamiento.hora_salida_furgoneta ? `\nSalida: ${entrenamiento.hora_salida_furgoneta.slice(0, 5)} h` : ''}
              </Text>
            </View>
          </TouchableOpacity>
        )}
      ...
      <View style={{ paddingBottom: 20 }}>
        <TouchableOpacity
          style={[styles.button, styles.botonAzul]}
          onPress={() => navigation.navigate('ModificarEntrenamiento', { entrenamiento })}
        >
          <Text style={styles.textoBoton}>MODIFICAR DATOS</Text>
        </TouchableOpacity>
        <TouchableOpacity style={[styles.button, styles.botonRojo]} onPress={handleEliminar}>
          <Text style={styles.textoBoton}>ELIMINAR ENTRENAMIENTO</Text>
        </TouchableOpacity>
      </View>
      ...
      <Modal visible={modalAsistenciaVisible} transparent animationType="fade">
        <View style={modalStyles.overlay}>
          <View style={modalStyles.content}>
            <Text style={modalStyles.titulo}>Inscritos</Text>
            ...
            <TouchableOpacity onPress={() => setModalAsistenciaVisible(false)} style={[modalStyles.botonCerrar, {backgroundColor: '#0369A1'}]}>
...
// --- ESTILOS DE LA CITACIÓN DE LA FURGONETA (IDÉNTICOS AL PADRE) ---
...
const cardStyles = StyleSheet.create({
  ...
  flecha: { fontSize: 18, color: '#9CA3AF', fontWeight: 'bold', marginLeft: 10 },
});
...
const modalStyles = StyleSheet.create({
  ...
  subtitulo: { fontSize: 12, color: '#6B7280', textAlign: 'center', marginBottom: 15 },
  filaPasajero: { flexDirection: 'row', justifyContent: 'space-between', paddingVertical: 12, borderBottomWidth: 1, borderBottomColor: '#F3F4F6' },
