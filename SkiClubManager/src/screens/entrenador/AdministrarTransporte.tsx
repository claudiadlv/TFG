...
export default function AdministrarTransporte() {
  ...
  const cargarViajes = useCallback(async () => {
    try {
      ...
      const res = await fetch(`${API_URL}/transporte/entrenador`, {
        method: 'GET',
        headers: {
      ...
      console.error('❌ Error cargando viajes de transporte:', e);
      setError('No se pudieron cargar los datos de transporte.');
      setViajes([]);
    } finally {
      setLoading(false);
    }
  ...
  const handleVerPasajeros = async (viajeId: string) => {
    ...
    try {
      const token = await AsyncStorage.getItem('accessToken');
      const response = await fetch(`${API_URL}/eventos/viaje/${viajeId}/pasajeros`, {
        ...
        ListEmptyComponent={
          <View>
            <Text style={styles.noDataText}>
              {busqueda ? 'No hay resultados que coincidan con tu búsqueda.' : 'No hay autobuses programados para tus categorías.'}
            </Text>
          </View>
        }
        renderItem={({ item }) => {
          ...
          // Formateador manual de fecha limpia
...
const styles = StyleSheet.create({
  ...
  subheader: { fontSize: 16, fontWeight: '600', color: '#333', marginTop: 15, marginBottom: 10 },
  ...
  categoriaText: { fontSize: 13, color: '#475569', backgroundColor: '#F1F5F9', padding: 6, borderRadius: 6, marginVertical: 6 },
