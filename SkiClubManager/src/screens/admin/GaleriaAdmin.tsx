...
export default function GaleriaAdminScreen() {
  ...
  // 1. CARGA DE TODAS LAS FOTOS Y CATEGORÍAS
  const cargarFotosDesdeBD = async () => {
    try {
      ...
      if (res.ok) {
        ...
        console.log('📥 [Front Admin] Datos crudos del backend:', jsonResponse);
        
        const fotosRaw = jsonResponse.fotos ? jsonResponse.fotos : [];
        const catsRaw = jsonResponse.categoriasVisibles ? jsonResponse.categoriasVisibles : ['General'];
        ...
        setCategoriasDisponibles(catsLimpias);

        if (catsLimpias.length > 0 && !catsLimpias.includes(categoriaActiva)) {
          setCategoriaActiva(catsLimpias[0]);
        }
      }
    } catch (err) {
  ...
  // 2. SUBIDA MÚLTIPLE ESPECIFICANDO CATEGORÍA
  const handleAddPhoto = async () => {
    Alert.alert(
      'Subir Imagen',
      `¿A qué categoría deseas asignar las fotos?`,
      [
        { text: `Pestaña Actual (${categoriaActiva})`, onPress: () => procesarSubidaCarrete(categoriaActiva) },
        { text: 'Forzar "General"', onPress: () => procesarSubidaCarrete('General') },
      ...
      for (let i = 0; i < assets.length; i++) {
        ...
        try {
          ...
          await fetch(`${API_URL}/api/fotos`, {
            ...
            body: JSON.stringify({ ruta: uri, fecha: fechaMetadato, categoria_id: categoriaAsignada }),
          });
        } catch (err) {
          console.log('Error en bucle de subida del Admin:', err);
        }
      }
      cargarFotosDesdeBD();
    });
  ...
  // 3. INTERACCIÓN Y ELIMINACIÓN ASOCIADA
  const handlePhotoPress = (photoId: string, uri: string) => {
    if (selectedPhotoIds.length > 0) {
      ...
      Alert.alert(
        ...
        '¿Qué acción deseas realizar sobre esta foto?',
        [
          { text: 'Descargar', onPress: () => downloadImage(uri) },
          { text: 'Eliminar del Club', onPress: () => confirmarEliminacion([photoId]), style: 'destructive' },
          { text: 'Cancelar', style: 'cancel' },
        ]
      );
    }
  ...
  const confirmarEliminacion = (ids: string[]) => {
    Alert.alert(
      'Eliminar como Admin',
      `¿Estás seguro de que quieres borrar permanentemente estas ${ids.length} fotos? Esto afectará a todos los usuarios.`,
      [
        { text: 'Sí, borrar del Club', onPress: () => eliminarFotosEnLote(ids), style: 'destructive' },
        { text: 'Cancelar', style: 'cancel' },
      ]
    );
  };
  ...
  const eliminarFotosEnLote = async (ids: string[]) => {
    ...
    try {
      ...
      console.log('Error al eliminar lote admin:', err);
      cargarFotosDesdeBD(); 
    }
  };
  ...
  // 5. AGRUPACIÓN FILTRADA CON BUSCADOR CRONOLÓGICO NORMALIZADO
  ...
  const seccionesAdmin = agruparFotosPorMes();
  ...
  return (
    <View style={styles.container}>
      ...
      {selectedPhotoIds.length > 0 ? (
        <View style={{ flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', backgroundColor: '#FFEBEE', padding: 12, borderRadius: 8, margin: 10 }}>
          <Text style={{ fontSize: 16, fontWeight: 'bold', color: '#C62828' }}>
            {selectedPhotoIds.length} seleccionadas (Acción Admin)
          </Text>
          ...
          {categoriasDisponibles.map((cat, index) => {
            ...
            return (
              <TouchableOpacity 
                ...
                style={{ paddingVertical: 6, paddingHorizontal: 12, backgroundColor: isActive ? '#2C3E50' : 'transparent', borderRadius: 20, marginBottom: 4, marginRight: 4 }}
              >
      ...
      <View style={{ flexDirection: 'row', alignItems: 'center', backgroundColor: '#F0F3F4', borderRadius: 10, marginHorizontal: 15, paddingHorizontal: 10, marginBottom: 15 }}>
        ...
        <TextInput
          placeholder={`Buscar recuerdos en categoría ${categoriaActiva}...`}
      ...
      {seccionesAdmin.length === 0 ? (
        <View style={{ flex: 1, justifyContent: 'center', alignItems: 'center', padding: 20 }}>
          ...
          <Text style={{ color: '#7F8C8D', fontSize: 14, textAlign: 'center' }}>
            No hay fotos subidas en la categoría "{categoriaActiva}".
          </Text>
        </View>
      ) : (
        <SectionList
