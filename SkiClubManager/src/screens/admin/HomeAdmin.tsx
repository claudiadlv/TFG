...
export default function HomeEntrenador() {
  ...
  const cargarEventos = async () => {
    try {
      ...
      const res = await fetchWithAuth(`${API_URL}/eventos/todos`, token);
      if (!res.ok) {
      ...
      const eventosValidos = normalizados.filter((evento) => {
        const d = new Date(evento.fecha); d.setHours(0,0,0,0);
  ...
  const renderItem = ({ item }: { item: Evento }) => {
    ...
    return (
      <TouchableOpacity onPress={() => navigation.navigate('DetalleEntrenamiento', { entrenamiento: item })}>
