...
export default function HomePadre() {
  ...
  const cargarEventos = async () => {
    try {
      ...
      // Modificado el endpoint para obtener los eventos donde el padre ha apuntado a sus hijos
      const res = await fetchWithAuth(`${API_URL}/eventos/apuntados`, token);
      if (!res.ok) {
      ...
      const eventosValidos = normalizados
        .filter((evento) => {
          ...
          return fechaEvento.getTime() >= hoy.getTime(); // Filtrar eventos para el día de hoy en adelante
        })
        .sort((a, b) => {
          const fechaA = new Date(a.fecha);
          const fechaB = new Date(b.fecha);
          return fechaA.getTime() - fechaB.getTime(); // Ordenar eventos por fecha ascendente
        });

      setEventos(eventosValidos);
    } catch (error) {
      console.error('Error al cargar eventos padre:', error);
      setEventos([]);
    } finally {
      setLoading(false);
      setRefreshing(false);
  ...
  const onRefresh = useCallback(() => { setRefreshing(true); cargarEventos(); }, []);
  ...
  const indice = useMemo<Indexado[]>(() => {
    return eventos.map(e => {
      ...
      const discExpanded = disciplina ? expandirSinonimos([disciplina], SINONIMOS_DISC).join(' ') : '';
      const keywords = [
        tipo,
        tipoExpanded,
        categoria,
  ...
  const renderItem = ({ item }: { item: Evento }) => {
    ...
    const categoriasMostrar = Array.isArray(item.categoria) ? item.categoria.join(', ') : item.categoria;
    return (
      <TouchableOpacity onPress={() => navigation.navigate('DetalleEventoPadreDesapuntar', { entrenamiento: item })}>
        <View style={[styles.card, { backgroundColor: color }]}>
          <Text style={styles.cardTitle}>{titulo}</Text>
          <View style={styles.categoriaChip}>
            <Text style={styles.categoriaTexto}>{categoriasMostrar}</Text>
          </View>
  ...
  return (
    <View style={styles.container}>
      <Text style={styles.title}>Eventos de tus hijos</Text>
      <View style={styles.searchContainer}>
        <Icon name="search" size={20} color="#999" />
        <TextInput
          style={styles.input}
          placeholder="Buscar por tipo, disciplina, categoría o fecha (ej. 'entreno gs viernes 15/10')"
          value={busqueda}
