...
Como se puede observar en la Tabla~\ref{tab:duracion_fases_real}, el proyecto se compone de seis fases principales distribuidas a lo largo del segundo cuatrimestre. Es imperativo destacar que la fase de \textit{Documentación} posee un carácter transversal, desarrollándose de forma simultánea y paralela junto con las fases de diseño, desarrollo e implementación del sistema Full Stack.
    ...
    \textbf{Total materiales} & & \textbf{175 €} & \\ \hline
    \end{tabularx}
    ...
    Para cuantificar el esfuerzo de desarrollo, se ha realizado un registro de las horas invertidas en cada una de las fases del ciclo de vida del software, sumando un total de 580 horas de trabajo. El desglose detallado por bloques funcionales se presenta de forma estructurada en la tabla \ref{tab:estimacion_esfuerzo}.
    ...
    \hline
    \rowcolor[HTML]{EFEFEF} 
    \textbf{Bloque} & \textbf{Horas} & \textbf{Comentario} \\ \hline
    Análisis (RF, RNF, Casos de Uso) & 90 & Definición de requisitos del club y flujos de usuarios \\ \hline
    Diseño (Arquitectura, ER, UI) & 75 & Modelado de base de datos y mockups en Figma \\ \hline
    Backend (API, controladores, JWT) & 140 & Desarrollo de endpoints, seguridad y lógica en Node.js \\ \hline
    Frontend (Vistas, Context, Gráficos) & 165 & Maquetación adaptativa y lógica en React Native \\ \hline
    Integración y Pruebas & 35 & Validación de flujos de asistencia y gestión de transporte \\ \hline
    Memoria y diagramas & 75 & Redacción de la documentación técnica en LaTeX \\ \hline
    \textbf{Total} & \textbf{580} & \\ \hline
    \end{tabularx}
    \caption{Estimación de esfuerzo (horas).}
    \label{tab:estimacion_esfuerzo}
    \end{table}

    Como tarifa de referencia para el cálculo del coste laboral se toma un precio de 13 €/h. Este valor se obtiene a partir del salario medio orientativo de un desarrollador \textit{full-stack} júnior en España (24\,000 € brutos anuales), convertido a coste horario mediante la fórmula de jornada estándar: $24\,000 / (40 \times 48) \approx 12,5$ €/h (asumiendo una jornada de 40 h/semana y 48 semanas laborables al año), redondeado a 13 €/h. Por tanto, el coste laboral de referencia queda establecido en $580 \times 13 = 7\,540$ €.
    ...
    Google Gemini API & 25 € & Créditos consumidos en desarrollo \\ \hline
    Software & 0 € & Entorno open source sin costes de licencia \\ \hline
    Mano de obra (referencia) & 7\,540 € & 580 h $\times$ 13 €/h \\ \hline
    \textbf{Total estimado de replicación} & \textbf{7\,715 €} & \\ \hline
    \end{tabularx}
    \caption{Presupuesto total estimado del proyecto.}
    \label{tab:presupuesto_total}
    \end{table}

    En conclusión, el desembolso directo en servicios de terceros durante la ejecución real de este Trabajo de Fin de Grado fue de 25 € en créditos de API, mientras que la infraestructura de hardware utilizada aporta un coste de amortización proporcional de 150 € a lo largo de los cinco meses de desarrollo, tal como se desglosó previamente.
    ...
    \subsubsection{Definición de Requisitos y Flujos de Interacción}
        ...
        \textbf{6.} La aplicación móvil renderiza la lista de forma visual en la pantalla del entrenador. \\ \hline
        \textbf{Cursos Alternos} & 
        ...
        \textbf{5.} La aplicación móvil recibe los datos y los renderiza de forma ordenada en la interfaz del calendario. \\ \hline
        \textbf{Cursos Alternos} & 
        ...
        \end{table}        
        \newpage
        \subsubsection{Módulo de gestión de logisitica y transporte}
        ...
        \textbf{Resumen} & El usuario accede al apartado de transporte. El sistema realiza una consulta relacional y renderiza la lista de deportistas con plaza reservada. \\ \hline
        \textbf{Curso Normal} & 
        ...
        \textbf{Resumen} & El padre accede a transporte. El sistema lee su identidad mediante el token JWT, filtra en MySQL por sus hijos vinculados y renderiza los estados de reserva en la interfaz. \\ \hline
        \textbf{Curso Normal} & 
        ...
        \textbf{Postcondición} & El sistema renderiza el componente gráfico en la pantalla del dispositivo móvil. \\ \hline
        ...
        \textbf{7.} El sistema inserta los metadatos y la URL en la tabla \textit{fotografias} de la base de datos.\newline
        ...
        \textbf{4.} El backend recibe los parámetros y realiza una consulta filtrada en la tabla \textit{fotografias} de MySQL.\newline
        ...
        \texttt{eventos.js} & GET & \texttt{/filtrados} & Eventos segmentados por categorías del usuario logueado. \\ \hline
        ...
        La interfaz de usuario de la plataforma se ha desarrollado como una aplicación móvil nativa multiplataforma utilizando el framework React Native y TypeScript. La navegación interna y el flujo de pantallas se gestionan mediante la librería React Navigation, implementando una estrategia de enrutamiento dinámico basado en el estado de autenticación y el rol asignado al usuario (Administrador, Entrenador o Padre). 
